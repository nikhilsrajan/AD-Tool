/*
 * File:   SparseMatrix.h
 * Author: aravind P, with modifications from satyaprasad and nikhilsrajan
 *
 * Created on September 24, 2017, 1:05 AM
 */
...
template<class T>
void SparseMatrix<T>::operator = (SparseMatrix<T> u){
    this->m = u.m;
    this->n = u.n;
    this->row = u.row;
    this->col = u.col;
...
template<class T>
SparseMatrix<T> operator *(vector<T> u, vector<T> v){
    SparseMatrix<T> P(u.size(), v.size());
